...
function createCustomizationStore() {
  const { subscribe, set, update } = writable<CustomizationState>(initialState);
  ...
  return {
    ...
    getCustomizedItem: () => {
      let result: CustomizationState | null = null;
      ...
      // Add size
      const selectedSize = result.size.options.find(s => s.selected);
      if (selectedSize) {
        customizations.push({
          id: selectedSize.id,
